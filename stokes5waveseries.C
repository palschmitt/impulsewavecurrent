...
Class
    Foam::stokes5waveseries::sinestokes5waveseries
    virtual stokes5waveseries class, different theories should be implemented in derivations
\*---------------------------------------------------------------------------*/
#include "stokes5waveseries.H"
...
void Foam::stokes5waveseries::setTime(const scalar &Time)
{
	scalar PeriodSum=0.0;
	int i = 0;
	
	do {
		Time_ = Time-PeriodSum;
		PeriodSum=PeriodSum+PeriodList_[i];
		i=i+1;
		} while (PeriodSum <= Time);
	
	if (waveindex_!=i)//New wave
	{
		Period_=PeriodList_[i-1];
		waveHeight_= WaveHeightList_[i-1]; 
...
scalar Foam::stokes5waveseries::evalwavenumber()
{
	...
	while (fabs(left-right) > 0.001)
	{
		...
		if(rr*rl > 0.)
		{
			FatalErrorIn("Foam::stokes5waveseries::evalwavenumber")
...
vector Foam::stokes5waveseries::velocity(const vector & p)
{
...
scalar Foam::stokes5waveseries::Xi(const vector &p)
{
	return(	Depth_+(
...
scalar Foam::stokes5waveseries::p(const vector &p)
{
...
//Validity checks and info  for all monochromatic wave types derived from stokes5waveseries!!!
void Foam::stokes5waveseries::checkUrsell()
{
	...
	Info<<"Type  	  :	stokes5waveseries	"<<endl;
	Info<<"############################################################# "<<endl;
...
void Foam::stokes5waveseries::setCoeffs()
{
...
//***************************Constructor********************************
Foam::stokes5waveseries::stokes5waveseries(IOdictionary waveDict, vector gravity, scalar waterdensity):sinewave( waterdensity)
{
